  ...
  return (async () => {
    ...
    let intervalId = setInterval(async () => {
      ...
      try {
        ...
        let currentLink = '';
        for (const rec of inProgress.values()) { currentLink = rec.intendedUrl || rec.lastUrl || ''; break; }

        setState({ status: 'polling', message: `Processing: ${completedCount} done, ${inProgressCount} active, ${linksQueue.length} queued`, total: totalToProcess, completed: completedCount, current: currentLink });
