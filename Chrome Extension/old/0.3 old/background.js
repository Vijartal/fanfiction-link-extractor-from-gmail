...
function startPerTabPolling(maxConcurrent) {
  ...
  const confirmDelayMs = 10_000;   // per-tab stable delay for confirmation
  const requiredChecks = 2;        // need consecutive stable checks

  let startedAt = Date.now();

  // named interval function for cleaner restarts if needed
  let intervalId = null;
  intervalId = setInterval(async function pollCycle() {
    if (abortFlag) {
    ...
    // if target window got closed unexpectedly, abort
    if (!targetWinId) {
      clearInterval(intervalId);
    ...
    // timeout safety: if too long, proceed with what we have
    const maxWaitMs = 30 * 60_000; // 30 minutes
    if (Date.now() - startedAt > maxWaitMs) {
      ...
      try {
        ...
        setState({ status: 'error', message: 'Error getting tabs at timeout: ' + err.message });
        cleanup();
      }
      return;
    }
    ...
    try {
      ...
      // update UI counts: total known = completed + inProgress size + queue length
      ...
      // iterate tabs present and manage per-tab checks
      for (const tab of tabs) {
        if (abortFlag) break;
        if (!inProgress.has(tab.id)) continue; // only interested in tabs we created and track

        const rec = inProgress.get(tab.id);
        const currUrl = tab.url || '';

        // stable URL check: if tab.status is 'complete'
  ...
  setState({ status: 'processing', message: `Tab resolved: ${resolved} — reusing or closing tab and opening next`, completed: resolvedUrls.length });

  // remove tracking (we'll re-add for reused tab)
  inProgress.delete(tab.id);

  // If we have a next URL in queue, reuse this tab by updating its URL.
  if (linksQueue.length > 0) {
    const nextUrl = linksQueue.shift();

    // Ensure the target window still exists. If not, create a new popup window.
    let windowValid = true;
    try {
      await chrome.windows.get(targetWinId);
    } catch (err) {
      windowValid = false;
      if (await readDebugFlag()) console.warn('[LR] target window missing; recreating window for continued processing.');
    }
    ...
    if (!windowValid) {
      try {
        ...
        // register the new tab (first tab in window)
        await new Promise(r => setTimeout(r, 300));
        const tabsNow = await chrome.tabs.query({ windowId: targetWinId });
        ...
        // failed to create new window/tab, put URL back and continue
        linksQueue.unshift(nextUrl);
        if (await readDebugFlag()) console.error('[LR] Failed to recreate window/tab for', nextUrl, err);
      }
    } else {
      // Try to update the current tab to the next URL (preferred)
      try {
        await chrome.tabs.update(tab.id, { url: nextUrl });
        // re-register same tab id for tracking
        inProgress.set(tab.id, { intendedUrl: nextUrl, checks: 0, lastUrl: '' });
      } catch (err) {
        // If update fails (rare), fallback to creating a new tab in the popup
        if (await readDebugFlag()) console.warn('[LR] Failed to update tab, creating new tab instead:', err);
        try {
          const newTab = await chrome.tabs.create({ windowId: targetWinId, url: nextUrl });
          inProgress.set(newTab.id, { intendedUrl: nextUrl, checks: 0, lastUrl: '' });
        } catch (err2) {
          linksQueue.unshift(nextUrl);
          if (await readDebugFlag()) console.error('[LR] Failed to create fallback tab for', nextUrl, err2);
        }
        // try to close the original tab only if it's safe (we'll attempt to remove it)
        try { await chrome.tabs.remove(tab.id); } catch (e) {}
      }
    }
  } else {
    // No next URL: close this tab and proceed. Be careful: closing the last tab closes the window.
    try {
      await chrome.tabs.remove(tab.id);
    } catch (err) {
      if (await readDebugFlag()) console.warn('[LR] Failed to remove tab', tab.id, err);
    }
  }

  // If everything is processed (queue empty and no inProgress tabs), finalize
  if (linksQueue.length === 0 && inProgress.size === 0) {
    // stop polling externally (calling code clears interval). We just ensure finalization.
    try {
      clearInterval(intervalId); // note: intervalId must be in scope; if not, the outer function already clears it on completion
    } catch (e) {}
