...
function updateUI(s) {
  ...
  if (s.lastErrorSample) {
    // show shortened sample in debug
    appendLog('SAMPLE: ' + s.lastErrorSample.slice(0,800));
  }
}
...
function requestStatus() {
  chrome.runtime.sendMessage({ type: 'GET_STATUS' }, res => {
    ...
    if (s) updateUI(s);
    // show debug area if DEBUG flag true
...
chrome.runtime.onMessage.addListener(msg => {
  if (msg && msg.type === 'STATUS' && msg.state) {
    updateUI(msg.state);
    chrome.storage.local.get('DEBUG', data => {
      if (data.DEBUG) appendLog(JSON.stringify(msg.state).slice(0,400));
    });
  }
});
